...
class Ai:
    ...
    def do(self, state):
        ...
        self.astar = AStar(self.player.get_centre_pos(), self.state.ball.get_centre_pos(), self.astar_size,
                           blocked_rects)
    ...
    def get_adjacents(self, node):
        ...
        for y in xrange(int(node.pos[1] - 1), int(node.pos[1] + 1 + 1)):
            for x in xrange(int(node.pos[0] - 1), int(node.pos[0] + 1 + 1)):
                if not (x == node.pos[0] and y == node.pos[1]) and (x >= 0 and y >= 0) and (
                        x < self.sizex and y < self.sizey) and not ([x, y] in self.blocked_poses):
        ...
        while len(self.openlist) > 0:
            ...
            for node in self.get_adjacents(next_node):
                ...
                if not (node in self.openlist):
                    ...
                    if node.diagonal:
                        node.g += self.unit_diagonal_cost
                    else:
                        node.g += self.unit_cost
                    node.f = node.g + node.h
                    self.openlist.append(node)
                else:
                    new_cost = next_node.g
                    if node.diagonal:
                        new_cost += self.unit_diagonal_cost
                    else:
